...
import { FiTrash2, FiMinus, FiPlus, FiArrowRight, FiX, FiChevronDown, FiTruck, } from "react-icons/fi";
import { LuBadgePercent } from "react-icons/lu";
import { fetchCart, removeFromCart, updateCartQuantity, type CartApiItem, } from "../services/cartApi";
type StoredUser = {
    id?: number;
    name?: string;
    email?: string;
...
const normalizeCartItem = (item: CartApiItem, businessAccount: boolean): CartLine => {
    const images = parseImages(item.images);
    const fallbackImage = item.front_image_url ||
        item.image_url ||
        item.main_image_url ||
        item.back_image_url ||
        "/placeholder.svg";
    ...
    const customPayload = item.custom_payload && typeof item.custom_payload === "object"
        ? item.custom_payload
        : {};
    return {
        ...
        weightKg: numberOrNull(item.weight_kg ?? customPayload.weight_kg ?? item.weight),
        ...
        hsnCode: String(item.hsn_code || item.hsnCode || customPayload.hsn_code || "").trim(),
        hsnPercentage: numberOrNull(item.hsn_percentage ??
            item.hsnPercentage ??
...
export default function Cart() {
    ...
    const updateQuantity = async (item: CartLine, delta: number) => {
        ...
        try {
            ...
            setCartItems((prev) => prev.map((line) => getItemKey(line) === key ? { ...line, quantity: nextQuantity } : line));
        }
        catch (err: any) {
    ...
    if (!userId) {
        return (<div className="v1-cart min-h-screen bg-gray-50 py-10 font-montserrat">
    ...
    if (loading) {
        return (<div className="v1-cart min-h-screen bg-gray-50 pt-10 pb-20 font-montserrat flex items-center justify-center">
        <p className="text-gray-500 font-medium">Loading cart...</p>
      </div>);
    }
    return (<div className="v1-cart min-h-screen bg-gray-50 pt-4 md:pt-10 pb-40 md:pb-20 font-montserrat flex flex-col items-center">
      <div className="w-full md:px-4 max-w-[1440px]">
        ...
        {error ? (<div className="mx-4 md:mx-8 mb-4 rounded-md bg-red-50 border border-red-200 px-4 py-3 text-sm font-medium text-red-700">
            {error}
          </div>) : null}

        {cartItems.length > 0 ? (<div className="flex flex-col lg:flex-row gap-4 md:gap-10 w-full px-0 md:px-8">
            <div className="flex-1 w-full">
              <div className="bg-white md:rounded-xl shadow-sm border-y md:border border-gray-100 overflow-hidden">
                ...
                return (<div key={key} className="p-4 md:p-6 flex flex-row gap-4 md:gap-6 bg-white relative group">
                        <Link to={`/product/${encodeURIComponent(String(item.designCode ||
                        item.productId ||
                        item.variantId ||
                        item.id))}`} className="w-[90px] h-[120px] md:w-32 md:h-40 shrink-0 bg-gray-50 rounded-[4px] md:rounded-lg overflow-hidden">
                          <img src={item.images[0] || "/placeholder.svg"} alt={item.title} className="w-full h-full object-cover object-top"/>
                        </Link>
                        ...
                        item.productId ||
                        item.variantId ||
                        item.id))}`} className="text-[12px] md:text-lg font-normal md:font-bold text-gray-500 md:text-gray-900 hover:text-primary transition-colors line-clamp-2 md:line-clamp-1 leading-snug">
                                {item.title}
                              </Link>

                              {item.patternType ? (<p className="text-xs md:text-xs text-gray-500 mt-1 uppercase tracking-wide">
                                  {item.patternType}
                                </p>) : null}
                              {item.stock > 0 ? (<p className="md:hidden text-xs text-[#ff8c4b] mt-1.5">
                                  Hurry! Only {item.stock} Left
                                </p>) : null}
                              <p className="md:hidden text-xs text-gray-600 font-medium flex items-center gap-1 mt-1">
                            ...
                            <div className="hidden md:block text-right">
                              ...
                              {item.originalPrice && item.originalPrice > item.price && (<p className="text-sm text-gray-400 line-through">
                                  ₹{item.originalPrice}
                                </p>)}
                            </div>

                            <button onClick={() => removeItem(item)} disabled={disabled} aria-label="Remove item" className="absolute top-4 right-4 md:hidden p-1 text-gray-400 hover:text-red-500 transition-colors disabled:opacity-50">
                              <FiX size={18}/>
                            </button>
                          </div>

                          <div className="v1-cart-options flex items-end md:items-center justify-between mt-3 md:mt-6">
                            ...
                            <div className="md:hidden text-right flex flex-col items-end">
                              <div className="flex items-end gap-1.5">
                                ...
                                {item.originalPrice && item.originalPrice > item.price && (<span className="text-[11px] text-gray-400 line-through mb-[2px]">
                                    ₹{item.originalPrice}
                                  </span>)}
                              </div>
                              {item.originalPrice && item.originalPrice > item.price && (<span className="text-[10px] text-[#009b4d] font-semibold mt-1 tracking-wide">
                                  You saved ₹{item.originalPrice - item.price}
                                </span>)}
                            </div>

                            <button onClick={() => removeItem(item)} disabled={disabled} className="hidden md:flex items-center gap-2 text-sm font-semibold text-red-500 hover:text-red-600 transition-colors disabled:opacity-50">
                              <FiTrash2 size={16}/>
                              <span>Remove</span>
                            </button>
                          </div>
              ...
              <div className="lg:hidden mt-4 bg-white border-y border-gray-100 flex flex-col">
                <details className="group overflow-hidden">
                  <summary className="flex justify-between items-center font-medium p-4 cursor-pointer outline-none marker:content-none [::-webkit-details-marker]:hidden">
                    ...
                    <FiChevronDown size={18} className="text-gray-500 group-open:rotate-180 transition-transform"/>
                  </summary>
            ...
            <div className="hidden lg:block w-full lg:w-[380px] shrink-0">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-28">
                ...
                <button type="button" onClick={handleCheckout} className="w-full py-4 bg-primary text-black font-bold tracking-widest text-[13px] uppercase rounded-sm hover:scale-[1.02] transition-transform flex items-center justify-center gap-2 mt-2">
                  Proceed to Checkout <FiArrowRight size={18}/>
                </button>

                <div className="mt-5 text-center">
                  <Link to="/collections" className="text-sm font-semibold text-gray-500 hover:text-black hover:underline">
                    Continue Shopping
                  </Link>
                </div>
              </div>
            </div>
          </div>) : (<div className="px-4 md:px-8 w-full mt-4">
      ...
      {cartItems.length > 0 && (<div className="v1-cart-actions lg:hidden fixed bottom-0 left-0 w-full z-50 bg-white border-t border-gray-200/60 shadow-[0_-4px_25px_rgba(0,0,0,0.08)]">
          {totalDiscount > 0 && (<div className="bg-[#00b259] text-white text-[11px] font-bold py-2 text-center flex items-center justify-center gap-1.5 tracking-wide">
          ...
          <div className="px-4 py-3 flex justify-between items-center gap-4">
            ...
            <button type="button" onClick={handleCheckout} className="bg-[#fdd835] text-[#2c2c2c] min-w-0 flex-1 sm:min-w-[200px] font-extrabold px-6 py-3.5 rounded-[4px] text-[13px] tracking-wider uppercase flex items-center justify-center transition-opacity hover:opacity-90">
              Proceed
            </button>
          </div>
        </div>)}
