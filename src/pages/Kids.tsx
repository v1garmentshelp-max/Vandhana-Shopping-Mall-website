...
const productDesignIdentity = (product: Product) => {
    ...
    if (designCode && colour)
        return `design|${designCode}|colour|${colour}`;
    if (designCode)
...
const getCategorySearchText = (category: any) => normalizeText(`${category?.name || ""} ${category?.categoryName || ""} ${category?.category_name || ""} ${category?.categoryPath || ""} ${category?.category_path || ""} ${category?.slug || ""}`);
...
const filterProductsByAudience = (products: Product[], categories: StorefrontCategory[], audience: KidsAudience) => {
    ...
    const categoryMatches = dedupeByDesign(products.filter((product: any) => categoryIds.has(getProductCategoryId(product))));
...
const Kids = () => {
    ...
    return (<div className="v1-store-page w-full bg-white">
      ...
      <div className="flex justify-center gap-3 px-4 py-5">
        <button type="button" onClick={() => setActiveAudience("boys")} className={`min-w-32 rounded-full border px-6 py-3 text-sm font-semibold transition-colors ${activeAudience === "boys" ? "border-secondary bg-secondary text-white" : "border-neutral-300 bg-white text-neutral-800 hover:border-secondary"}`}>
          Kids-Boys
        </button>
        <button type="button" onClick={() => setActiveAudience("girls")} className={`min-w-32 rounded-full border px-6 py-3 text-sm font-semibold transition-colors ${activeAudience === "girls" ? "border-secondary bg-secondary text-white" : "border-neutral-300 bg-white text-neutral-800 hover:border-secondary"}`}>
          Kids-Girls
        </button>
      </div>
