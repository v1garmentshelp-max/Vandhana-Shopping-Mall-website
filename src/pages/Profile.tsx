...
const firstProductImage = (product: Product | any, variant: ProductVariant | any) => {
    ...
    return text(variant?.front_image_url ||
...
const enrichOrders = async (orders: OrderRecord[]): Promise<OrderRecord[]> => {
    const variantIds = new Set(orders
        ...
        return orders.map((order) => ({
            ...
            items: order.items.map((item) => {
                ...
                return {
                    ...
                    pattern_type: text(variant?.pattern_type ||
                        variant?.patternType ||
                        product?.pattern_type ||
                        product?.patternType) || item.pattern_type,
                    pattern_code: text(variant?.pattern_code ||
                        variant?.patternCode ||
                        product?.pattern_code ||
                        product?.patternCode) || item.pattern_code,
                    size: text(variant?.size) || item.size,
                    colour: text(variant?.colour || variant?.color) || item.colour,
                    ean_code: text(variant?.ean_code || variant?.eanCode || variant?.barcode) ||
                        item.ean_code,
                    image_url: firstProductImage(product, variant) || item.image_url,
                    product_name: text(product?.title || product?.name || product?.product_name) ||
                        item.product_name,
                    brand_name: text(product?.brand || product?.brand_name) || item.brand_name,
                };
            }),
        }));
    }
...
const buildAddressKey = (address: Record<string, any>) => JSON.stringify({
    line1: address?.line1 || "",
    line2: address?.line2 || "",
    city: address?.city || "",
    state: address?.state || "",
...
export default function Profile() {
    ...
    const loadProfileAndOrders = async (email: string) => {
        ...
        try {
            ...
            setForm({
                firstName: nextProfile.name?.split(" ").slice(0, -1).join(" ") ||
                    nextProfile.name ||
                    "",
                lastName: nextProfile.name?.split(" ").slice(-1).join(" ") === nextProfile.name
                    ? ""
    ...
    const syncStoredUser = (nextProfile: CustomerProfile) => {
        ...
        const merged = {
            ...
            customer_type: nextProfile.type ?? current.customer_type ?? current.type,
        };
    ...
    const renderContent = () => {
        if (loading) {
            return (<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 min-h-[400px] flex items-center justify-center">
        ...
        switch (activeTab) {
            ...
            case "rewards":
                return (<RewardsTab wallet={rewardWallet} history={rewardHistory} loading={rewardLoading} error={rewardError} onRefresh={loadRewards}/>);
            case "profile":
                return (<ProfileTab profile={profile} form={form} setForm={setForm} onSubmit={handleSaveProfile} saving={savingProfile} message={message} error={error}/>);
            case "overview":
            default:
                return (<>
            <div className="bg-[#fff9ea] rounded-xl p-8 border border-[#feedc5] shadow-sm relative overflow-hidden mb-6 pt-10 pb-8">
              ...
              <button onClick={() => setActiveTab("profile")} className="w-full py-4 px-6 bg-[#fdd835] text-[#2c2c2c] font-extrabold text-[13px] tracking-wider uppercase rounded-[4px] hover:bg-[#f2c713] transition-colors">
                Edit Profile
              </button>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {statCards.map((card, i) => (<div key={i} onClick={() => {
                            if (card.id === "support") {
            ...
            <button onClick={handleLogout} className="w-full md:hidden flex items-center justify-center gap-2 mt-3 py-4 px-6 bg-red-500/20 text-red-500 font-extrabold text-[13px] tracking-wider uppercase rounded-[4px] hover:bg-red-500/30 transition-colors">
              <FiLogOut size={22}/>
    ...
    return (<div className="v1-profile min-h-screen bg-gray-50 pt-8 pb-20 font-montserrat flex justify-center">
      <div className="w-full max-w-[1440px] px-4 md:px-8 flex flex-col md:flex-row gap-6 md:gap-8">
        <div className="w-full md:w-72 shrink-0 hidden md:block">
          <div className="md:sticky md:top-28 bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm flex flex-col">
            {sidebarLinks.map((link, i) => (<div key={i} onClick={() => setActiveTab(link.id)} className={`flex items-center gap-4 px-6 py-5 cursor-pointer transition-all border-b border-gray-100 last:border-b-0 ${activeTab === link.id
                ? "text-[#4285f4] border-l-4 border-l-[#4285f4] bg-[#f8fbff]"
                : "hover:bg-gray-50 text-gray-600 border-l-4 border-l-transparent"}`}>
                {link.icon}
                <span className={`text-[15px] font-medium ${activeTab === link.id ? "font-bold" : ""}`}>
                  {link.title}
                </span>
              </div>))}
            <div onClick={handleLogout} className="flex items-center gap-4 px-6 py-5 cursor-pointer border-t border-gray-100 text-red-500 hover:bg-red-50 transition-colors border-l-4 border-l-transparent">
              <FiLogOut size={22}/>
              <span className="text-[15px] font-bold">Logout</span>
            </div>
          </div>
        ...
        <div className="min-w-0 flex-1 flex flex-col w-full">
          <nav aria-label="Account sections" className="v1-account-nav md:hidden">
            {sidebarLinks.map((link) => (<button type="button" key={link.id} onClick={() => setActiveTab(link.id)} aria-current={activeTab === link.id ? "page" : undefined}>
                {link.title}
              </button>))}
    ...
    return (<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8 animate-in fade-in duration-300">
      ...
      {!orders.length ? (<div className="rounded-lg border border-dashed border-gray-200 p-10 text-center">
          ...
          {orders.map((order) => (order.items.length
                ? order.items
                : [{ qty: 1, price: Number(order.totals?.payable || 0) }]).map((item, index) => (<div key={`${order.id}-${item.variant_id || index}`} className="flex gap-6 p-4 border border-gray-100 rounded-lg hover:shadow-[0_4px_20px_rgba(0,0,0,0.04)] transition-shadow bg-white">
                  <div className="shrink max-w-[30%] aspect-[3/4] bg-gray-50 rounded-md overflow-hidden">
                    <img src={item.image_url ||
                    "https://via.placeholder.com/300x400?text=Product"} alt={item.product_name || "Product"} className="w-full h-full object-cover object-top"/>
                  </div>
                  ...
                  <div className="flex-1 flex justify-between flex-col">
                    <div>
                      ...
                      {item.pattern_type ? (<p className="text-[11px] text-gray-400 mt-1 uppercase tracking-wider">
                          {item.pattern_type}
                        </p>) : null}
...
function RewardsTab({ wallet, history, loading, error, onRefresh, }: {
    ...
    return (<div className="flex flex-col gap-5 animate-in fade-in duration-300">
      <div className="bg-[#111111] rounded-xl shadow-sm border border-black p-6 md:p-8 text-white overflow-hidden relative">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          ...
          <button type="button" onClick={() => void onRefresh()} className="self-start px-4 py-2.5 rounded-lg border border-[#fdd835]/40 bg-[#fdd835]/10 text-[#fdd835] text-xs font-bold hover:bg-[#fdd835]/15 transition-colors">
            Refresh
          </button>
        </div>
      </div>

      {error ? (<div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">
          {error}
        </div>) : null}

      {wallet?.hurryUp ? (<div className="rounded-xl border border-orange-200 bg-orange-50 p-4 flex items-start gap-3">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
        ...
        {!wallet?.activeLots.length ? (<div className="rounded-lg border border-dashed border-gray-200 p-8 text-center">
            <p className="text-sm text-gray-500 font-medium">
              No active reward points are available.
            </p>
          </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {wallet.activeLots.map((lot) => (<div key={lot.id} className="rounded-xl border border-[#f0e3a7] bg-[#fffdf6] p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  {lot.daysRemaining != null && lot.daysRemaining <= (wallet.warningDays || 10) ? (<span className="rounded-full bg-orange-100 px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider text-orange-700">
                      Hurry Up
                    </span>) : null}
                </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
        ...
        {!history.length ? (<div className="rounded-lg border border-dashed border-gray-200 p-8 text-center">
            <p className="text-sm text-gray-500 font-medium">
            ...
            <table className="w-full min-w-[720px] border-collapse">
              ...
              <tbody>
                ...
                return (<tr key={String(item.id)} className="border-b border-gray-50 last:border-b-0">
...
function AddressesTab({ addresses }: {
    ...
    return (<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8 animate-in fade-in duration-300">
      ...
      {!addresses.length ? (<div className="rounded-lg border border-dashed border-gray-200 p-10 text-center">
          <p className="text-gray-500 font-medium">No addresses found yet.</p>
        </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {addresses.map((item) => (<div key={item.key} className="p-6 border border-gray-200 rounded-lg relative hover:border-[rgba(253,216,53,0.5)] hover:bg-[#fffdf6] transition-colors group">
...
function ProfileTab({ profile, form, setForm, onSubmit, saving, message, error, }: {
    ...
    return (<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8 animate-in fade-in duration-300">
      ...
      {message ? (<div className="mb-5 rounded-md bg-green-50 border border-green-200 px-4 py-3 text-sm font-medium text-green-700">
          {message}
        </div>) : null}
      ...
      <form className="max-w-xl flex flex-col gap-6" onSubmit={onSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" value={form.firstName} onChange={(e) => setForm((prev) => ({ ...prev, firstName: e.target.value }))} className="w-full px-4 py-3.5 rounded-md border border-gray-200 text-[14px] text-gray-900 font-medium focus:outline-none focus:ring-2 focus:ring-[#fdd835] focus:border-transparent transition-all shadow-sm"/>
          </div>
          ...
          <div>
            ...
            <input type="text" value={form.lastName} onChange={(e) => setForm((prev) => ({ ...prev, lastName: e.target.value }))} className="w-full px-4 py-3.5 rounded-md border border-gray-200 text-[14px] text-gray-900 font-medium focus:outline-none focus:ring-2 focus:ring-[#fdd835] focus:border-transparent transition-all shadow-sm"/>
          </div>
        </div>
        ...
        <div>
          ...
          <input type="email" value={profile.email} disabled className="w-full px-4 py-3.5 bg-gray-50 text-gray-500 text-[14px] font-medium rounded-md border border-gray-200"/>
        ...
        <div>
          ...
          <div className="flex">
            ...
            <input type="tel" value={form.mobile} onChange={(e) => setForm((prev) => ({ ...prev, mobile: e.target.value }))} className="w-full px-4 py-3.5 rounded-r-md border border-gray-200 text-[14px] text-gray-900 font-medium focus:outline-none focus:ring-2 focus:ring-[#fdd835] focus:border-transparent transition-all shadow-sm"/>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-gray-100 mt-2">
          ...
          <div className="flex flex-col gap-4">
            <input aria-label="Current Password" type="password" placeholder="Current Password" value={form.currentPassword} onChange={(e) => setForm((prev) => ({
            ...prev,
            ...
            <input aria-label="New Password" type="password" placeholder="New Password" value={form.newPassword} onChange={(e) => setForm((prev) => ({ ...prev, newPassword: e.target.value }))} className="w-full px-4 py-3 placeholder:text-gray-400 rounded-md border border-gray-200 text-[14px] font-medium focus:outline-none focus:ring-2 focus:ring-[#fdd835] focus:border-transparent transition-all shadow-sm"/>
          </div>
        </div>

        <div className="mt-4">
          <button type="submit" disabled={saving} className="px-10 py-3.5 bg-[#fdd835] text-[#2c2c2c] font-extrabold text-[12px] tracking-widest uppercase rounded hover:bg-[#f2c713] transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed">
