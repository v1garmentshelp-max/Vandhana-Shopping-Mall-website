...
const fallbackCategories =
  categoriesJson as StorefrontCategory[];
...
const getImagePairFromSource = (source: any) => {
  ...
  const front = imageUrlFromRecord(
    source?.front_image_url ||
      source?.frontImageUrl ||
      source?.front_url ||
      source?.frontUrl ||
      imageByType(raw, ["front", "primary"]) ||
      source?.main_image_url ||
      source?.mainImageUrl ||
      source?.image_url ||
      source?.imageUrl ||
      imageByType(raw, ["main", "default"]) ||
      raw[0],
  );

  const back = imageUrlFromRecord(
...
const normalizeVariant = (
  ...
  const b2cDiscount =
    getB2CDiscount(variant, row);

  const b2bDiscount =
    getB2BDiscount(variant, row);
  ...
  const images = uniqueImages([
    variantImages[0] || rowImages[0],
    variantImages[1] || rowImages[1],
  ]).slice(0, 2);
  ...
  const parentCategoryId = cleanText(
    firstValue(
  ...
  const parentCategoryName = cleanText(
    firstValue(
  ...
  const parentCategorySlug = cleanText(
    firstValue(
...
const rawProductKey = (row: any) => {
  ...
  if (productId) {
    return `product:${productId}|category:${categoryId || "none"}|pattern:${patternCode || "none"}`;
  }
...
const sortVariantValues = (values: any[]) =>
...
const normalizeProductCards = (
  ...
  const usableVariants = variants.some(
    (variant: NormalizedVariant) =>
  ...
  const groups = colors.length
    ? colors.map((color) => ({
        color,
        variants: usableVariants.filter(
          (variant: NormalizedVariant) =>
            sameColor(
              variant.colour,
              color,
  ...
  const actualProductId = firstValue(
    row.product_id,
    row.productId,
    row.id,
    "",
  ...
  const allColors = sortVariantValues(
    variants.map(
      (variant: NormalizedVariant) =>
        variant.colour,
    ),
  ...
  return groups.map(
    ({
      ...
      const inventory = selectedGroup.reduce(
        (
          totals: {
            onHand: number;
            reserved: number;
      ...
      const b2cDiscount =
        selectedVariant?.b2c_discount_pct ||
        0;
...
const fetchFromBranchApi = async (
  ...
  const backendGender = options.gender
    ? toBackendGender(options.gender)
    : "";
...
const productIdentityKey = (product: any) => {
  ...
  if (productId) {
    return `product:${productId}|category:${categoryId || "none"}|pattern:${patternCode || "none"}|color:${color || "default"}`;
  }
...
export const fetchBranchProducts = async (
  ...
  let rows: any[] = [];
  let usedBranchEndpoint = false;

  try {
    rows = await fetchFromBranchApi(
      branchId,
      options,
    );

    usedBranchEndpoint = true;
  } catch {
    rows =
      await fetchFromProductsApi(branchId);
  }
  ...
  let products = mergeRawRows(rows)
    .flatMap((row) =>
      normalizeProductCards(row),
    )
    .filter((product) =>
      Boolean((product as any).id),
    );

  if (options.gender) {
    const targetGender = String(
      toGender(options.gender),
    ).toLowerCase();
