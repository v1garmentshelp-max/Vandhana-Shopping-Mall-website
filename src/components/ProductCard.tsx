...
const removeStartToken = (text: string, token: any) => {
    ...
    if (!rawNorm.startsWith(tokenNorm))
        return raw;
...
const cleanDisplayTitle = (props: any) => {
    const rawTitle = cleanText(props.title ||
        props.name ||
        props.product_name ||
        props.productName ||
        "Product");
    const brand = cleanText(props.brand ||
        props.brand_name ||
        props.brandName);
    const color = cleanText(props.colour ||
        props.color ||
        props.selectedColour ||
        props.selectedColor ||
        props.selected_colour ||
        props.selected_color);
    const categoryName = cleanText(props.categoryName ||
        props.category_name ||
        "");
    let title = rawTitle
        .replace(/['’]/g, "'")
...
const getImageType = (value: any) => {
    if (!value || typeof value === "string")
        return "";
    return String(value.image_type ||
        value.imageType ||
        value.type ||
        "")
        .trim()
...
export const ProductCardSkeleton: React.FC = () => {
    return (<div className="v1-product-card block w-full max-w-[400px] animate-pulse">
...
export const ProductCard: React.FC<ProductCardProps> = (props: any) => {
    const { id, productId, product_id, variantId, variant_id, primaryVariantId, primary_variant_id, designCode, design_code, routeKey, route_key, groupKey, group_key, variants, images, title, brand, price, originalPrice, isSale, frontImageUrl, front_image_url, backImageUrl, back_image_url, mainImageUrl, main_image_url, imageUrl, image_url, barcode, ean_code, eanCode, barcodes, ean_codes, showBrand = true, } = props;
    ...
    const displayBrand = useMemo(() => cleanDisplayBrand(brand ||
        props.brand_name ||
        props.brandName), [
        brand,
        props.brand_name,
    ...
    const resolvedImages = useMemo(() => {
        ...
        const front = firstValidImage([
            ...
            imageUrl,
            image_url,
        ], allowedImageCodes) || "/placeholder.svg";
        ...
        return {
            front,
            back: back && !sameImage(back, front)
                ? back
                : "",
        };
    }, [
    ...
    const firstVariant = useMemo(() => {
        ...
        return (variants.find((variant: any) => variant?.is_active !== false) ||
            variants[0] ||
            null);
    }, [variants]);
    const finalVariantId = variantId ||
        ...
        firstVariant?.id;
    const wishlistVariantId = useMemo(() => toPositiveNumber(finalVariantId), [finalVariantId]);
    const parentProductId = useMemo(() => {
        return (toPositiveNumber(productId || product_id) ||
            toPositiveNumber(firstVariant?.productId ||
    ...
    const discount = finalOriginalPrice &&
        ...
        ? Math.round(((finalOriginalPrice - finalPrice) /
            finalOriginalPrice) *
            100)
        : 0;
    const frontImg = frontFailed
        ...
        : resolvedImages.front ||
            "/placeholder.svg";
    const backImg = !backFailed &&
        resolvedImages.back &&
        !sameImage(resolvedImages.back, frontImg)
        ? resolvedImages.back
        : "";
    ...
    return (<Link to={`/product/${routeId}`} data-design-code={designCode ||
            design_code ||
            undefined} className={`v1-product-card block w-full max-w-[400px] cursor-pointer overflow-hidden ${hasBackImage ? "group" : ""}`}>
      <div className="relative aspect-[3/4] overflow-hidden rounded-xl bg-gray-100">
        {(isSale || discount > 0) && (<div className="absolute left-3 top-0 z-20 flex h-[22px] items-center justify-center rounded-b-sm bg-primary px-3 py-0.5 text-black">
            <span className="font-big-shoulders text-[0.82rem] font-bold uppercase leading-none">
              Sale
            </span>
          </div>)}

        <img src={frontImg} alt={displayTitle || title} loading="lazy" onError={() => setFrontFailed(true)} className={`relative z-10 h-full w-full object-cover object-top transition-all duration-500 ${hasBackImage
            ? "group-hover:scale-105 group-hover:opacity-0"
            : ""}`}/>

        {hasBackImage ? (<img src={backImg} alt={`${displayTitle || title} back`} loading="lazy" onError={() => setBackFailed(true)} className="absolute inset-0 z-10 h-full w-full object-cover object-top opacity-0 transition-all duration-500 group-hover:scale-105 group-hover:opacity-100"/>) : null}

        <button type="button" onClick={handleWishlistToggle} disabled={isUpdatingWishlist} aria-label={isWishlisted
            ? "Remove from wishlist"
            : "Add to wishlist"} className="absolute bottom-3 right-3 z-40 cursor-pointer rounded-full bg-white p-2.5 shadow-md transition-transform hover:scale-110 hover:bg-gray-100 disabled:opacity-60">
          <Heart size={18} className={isWishlisted
      ...
      <div className="mt-3 flex flex-col">
        {showBrand ? (<p className="truncate font-poppins text-[0.72rem] font-extrabold uppercase leading-tight tracking-wide text-gray-500 md:text-[0.78rem]">
            {displayBrand}
          </p>) : null}

        <h3 aria-label={displayTitle} title={displayTitle} className={`${showBrand ? "mt-1" : ""} line-clamp-2 font-big-shoulders text-[1.1rem] font-bold uppercase leading-[1.05] tracking-tight text-black md:text-[1.2rem]`}>
          {displayTitle}
        </h3>
