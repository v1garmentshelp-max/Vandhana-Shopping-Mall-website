...
const Hero: React.FC = () => {
    ...
    return (<section className="p-3 md:p-6">
      <div className="relative v1-hero-frame w-full h-[calc(100vh-10rem)] overflow-hidden bg-black text-white rounded-[40px]">
        ...
        {slides.map((slide, index) => (<div key={slide.id} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${index === currentSlide ? "opacity-100" : "opacity-0"}`}>
            <img src={slide.image} alt={slide.heading} className="w-full h-full object-cover opacity-100 object-top"/>
            <div className="absolute inset-0 bg-linear-to-t md:bg-linear-to-r from-black/90 to-black/10"/>
          </div>))}

        
        <div className="relative z-10 h-full md:max-w-7xl mx-auto flex flex-col justify-end md:justify-center
           px-6 md:px-12">
          <div className="max-w-3xl flex flex-col gap-8 md:gap-10 mb-36">
            
            <h1 key={currentSlide} className="text-5xl md:text-6xl font-poppins lg:text-8xl  leading-[0.9] tracking-tighter italic transition-all duration-700 animate-in fade-in slide-in-from-bottom-6">
              {slides[currentSlide].heading.split(",").map((text, i) => (<React.Fragment key={i}>
                  {text} {i === 0 && <br />}
                </React.Fragment>))}
            </h1>
        ...
        <div className="absolute bottom-12 left-0 w-full px-6 md:px-12 z-10">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="relative w-full h-[2px] bg-gray-700 mb-8 overflow-hidden">
              ...
              <div className="absolute top-0 h-full bg-white shadow-[0_0_8px_white]" style={{
            ...
            <div className="grid grid-cols-4 gap-4">
              {slides.map((slide, index) => (<div key={slide.id} onClick={() => {
