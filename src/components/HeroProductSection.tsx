...
const HeroProductSection: React.FC<HeroProductSectionProps> = ({
  ...
  // 1. Embla for the Main Swipable Image
  const [mainRef, mainApi] = useEmblaCarousel({ loop: true });

  // 2. Embla for the Thumbnail Navigation
  const [thumbRef, thumbApi] = useEmblaCarousel({
    containScroll: "keepSnaps",
    dragFree: true,
  });

  // Sync state when the main image is swiped
  ...
  // Sync Embla instances when activeProduct changes (Autoplay or Thumbnail click)
  useEffect(() => {
    const index = products.findIndex(
      (p) => p.images[0] === activeProduct.images[0],
    );
    if (index === -1) return;
          ...
          >
            ...
            <div className="overflow-hidden pb-4" ref={thumbRef}>
              <div className="flex gap-4">
                {products.map((product, index) => {
                  const isActive =
                    activeProduct.images[0] === product.images[0];
                  return (
                    <div
                      key={index}
