...
const getProductKey = (product?: Product | null, index = 0) => String(product?.designCode ||
    product?.design_code ||
    product?.routeKey ||
    product?.route_key ||
    product?.groupKey ||
    ...
    product?.id ||
    `product-${index}`).trim();
const getProductRoute = (product?: Product | null) => encodeURIComponent(getProductKey(product));
...
const HeroProductSection: React.FC<HeroProductSectionProps> = ({ products, className, }) => {
    const displayProducts = useMemo(() => dedupeProducts(Array.isArray(products) ? products : []), [products]);
    const [activeProduct, setActiveProduct] = useState<Product | null>(displayProducts[0] || null);
    ...
    const onSelect = useCallback(() => {
        if (!mainApi || !displayProducts.length)
            return;
    ...
    useEffect(() => {
        if (!activeProduct || !displayProducts.length)
            return;
    ...
    if (!displayProducts.length || !activeProduct)
        return null;
    const patternType = activeProduct.patternType || activeProduct.pattern_type || "";
    return (<section className={`v1-featured w-full bg-[#111111] py-6 md:py-10 overflow-hidden md:max-h-[calc(100vh-52px)] ${className || ""}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-12 items-center">
          <div className="order-2 lg:order-1 flex flex-col items-start text-white">
            <div key={getProductKey(activeProduct)} className="animate-in fade-in slide-in-from-left-4 duration-700">
              {patternType ? (<div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              <div className="mt-4 md:mt-10 flex items-center gap-4 font-source-sans">
                ...
                {activeProduct.originalPrice && activeProduct.originalPrice > activeProduct.price ? (<span className="text-3xl md:text-4xl text-gray-600 line-through tracking-tighter">
                    ₹{activeProduct.originalPrice}
                  </span>) : null}
              </div>
            </div>

            <Link to={`/product/${getProductRoute(activeProduct)}`} className="mt-4 md:mt-10 group flex items-center gap-3 bg-white text-black pl-1 pr-8 py-1 rounded-full transition-transform hover:scale-105 active:scale-95 cursor-pointer">
          ...
          <div className="order-1 lg:order-2 flex flex-col gap-6 md:px-10" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
            <div className="overflow-hidden rounded-[3rem] bg-[#1a1a1a]" ref={mainRef}>
              <div className="flex">
                {displayProducts.map((product, index) => (<div className="flex-[0_0_100%] aspect-5/6 md:aspect-square" key={getProductKey(product, index)}>
                    <img src={getProductImage(product)} alt={product.title} className="w-full h-full object-cover object-top transition-opacity duration-700"/>
                  </div>))}
              </div>
            </div>
            ...
            return (<button type="button" key={productKey} onClick={() => setActiveProduct(product)} className={`relative w-24 h-24 md:w-32 md:h-32 shrink-0 rounded-2xl md:rounded-3xl overflow-hidden cursor-pointer border-2 transition-all duration-500 ${isActive ? "border-white scale-95 opacity-100" : "border-transparent opacity-40 hover:opacity-70"}`} aria-label={`Show ${product.title}`}>
                      <img src={getProductImage(product)} alt={product.title} className="w-full h-full object-cover object-top"/>
                      {isActive && !isPaused ? (<span className="absolute bottom-0 left-0 h-1 bg-white animate-[progress_4s_linear]"/>) : null}
                    </button>);
        })}
              </div>
            </div>
