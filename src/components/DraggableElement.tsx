...
export default function DraggableElement({ element, isSelected, onSelect, onUpdate, containerRef, setNodeRef, onInteractionStart, onInteractionEnd, }: DraggableElementProps) {
    ...
    const startAction = (e: React.PointerEvent, type: "drag" | "resize" | "rotate") => {
        e.stopPropagation();
        onSelect(element.id);
    ...
    const handlePointerMove = (e: PointerEvent) => {
        if (!containerRef.current || !actionType.current)
            return;
        ...
        if (actionType.current === "drag") {
            ...
            if (containerRef.current) {
                ...
                const halfW = elW / 2;
                const halfH = elH / 2;
                newX = Math.max(Math.min(halfW, maxX / 2), Math.min(newX, Math.max(maxX - halfW, maxX / 2)));
                newY = Math.max(Math.min(halfH, maxY / 2), Math.min(newY, Math.max(maxY - halfH, maxY / 2)));
            }
    ...
    return (<div ref={(node) => {
            ...
            if (isSelected && setNodeRef)
                setNodeRef(node);
        }} onClick={(e) => e.stopPropagation()} onPointerDown={(e) => startAction(e, "drag")} className={`absolute cursor-move ${actionActive
            ? "outline-dashed outline-2 outline-gray-400 z-30"
            : isSelected
                ? "ring-2 ring-primary ring-offset-1 z-20"
                : "z-10 hover:ring-1 hover:ring-gray-300"}`} style={{
      ...
      <div className="w-full h-full" style={{
        ...
        {element.type === "text" ? (<div ref={textRef} contentEditable={isEditing} suppressContentEditableWarning onBlur={(e) => {
                setIsEditing(false);
                handleTextChange(e);
            }} onDoubleClick={(e) => {
                e.stopPropagation();
            ...
            }} onPointerDown={(e) => (isEditing ? e.stopPropagation() : undefined)} className="w-full h-full whitespace-pre-wrap break-all outline-none min-w-[20px] min-h-[20px] px-1">
            {element.content}
          </div>) : (<img src={element.content} alt="element" className="w-full h-full object-contain pointer-events-none"/>)}
      </div>

      
      {isSelected && !actionActive && (<>
          <div className="handler absolute -top-8 left-1/2 -translate-x-1/2 w-6 h-6 bg-white rounded-full border shadow flex items-center justify-center cursor-grab hover:bg-gray-50 text-gray-600" onPointerDown={(e) => startAction(e, "rotate")}>
            <RotateCw size={12}/>
          </div>

          <div className="handler absolute -bottom-2 -right-2 w-4 h-4 bg-primary rounded-full border-2 border-white shadow cursor-nwse-resize" onPointerDown={(e) => startAction(e, "resize")}/>
