...
const CollectionTabsContent = ({ title }: { title?: string }) => {
  ...
  // Automatically detect all Level 2 (leaf) categories to use as tabs tightly scoped to gender
  const tabs = useMemo(() => {
    ...
    const uniqueNames = Array.from(new Set(level2Cats.map((c) => c.name)));
    return ["ALL", ...uniqueNames];
  }, [preferredGender]);

  // Sticky Header / Tab Scroll Logic
  useEffect(() => {
    const handleScroll = () => {
  ...
  const currentProducts = useMemo(() => {
    ...
    if (activeTab !== "ALL") {
      ...
      return genderProducts.filter((p) => catIds.includes(p.categoryId));
    }

    // "ALL" tab advanced sorting:
    // 1. Recently Viewed
    // 2. Related to recently viewed (same category or gender)
    // 3. Everything else
    ...
    // Sort recently viewed exactly as they appear in history
    recentlyViewed.sort(
      (a, b) =>
    ...
    const related = genderProducts.filter(
      (p) =>
        !recentlyViewedIds.includes(p.id) && rvCategoryIds.has(p.categoryId),
    );
        ...
        if (entries[0].isIntersecting && !loadingRef.current) {
          if (visibleCount < currentProducts.length) {
            ...
            timeoutId = setTimeout(() => {
              ...
              setLoading(false);
            }, 400); // Simulate realistic loading
          }
        }
      },
